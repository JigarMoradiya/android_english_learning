...
import com.example.myapplication.main.age_group.phonics.l12_magic_e.view_model.MagicEListenEntry
import com.example.myapplication.main.age_group.phonics.l12_magic_e.view_model.MagicEListenUiState
import com.example.myapplication.main.age_group.phonics.l12_magic_e.view_model.MagicEListenViewModel
...
@Composable
fun MagicEListenPage(
    navController: NavController,
    viewModel: MagicEListenViewModel = hiltViewModel()
) {
    val uiState = viewModel.uiState
    val entry   = viewModel.currentEntry
    val accent  = entry?.accentColor ?: Color(0xFF880E4F)
    val shadow  = entry?.shadowColor ?: Color(0xFF6A1B9A)
        ...
        Column(
            ...
            Row(
                ...
                Column(
                    ...
                    if (entry != null) {
                        Text(
                            text = "'${entry.groupVowel}' says its name '${entry.longLabel}'!",
...
@Composable
private fun MagicEWordCard(
    entry: MagicEListenEntry,
    uiState: MagicEListenUiState,
    wordIndex: Int,
    ...
    AnimatedContent(
        ...
        // Per-word arc position state
        var canvasRootX by remember { mutableFloatStateOf(0f) }
        var vowelLeftX  by remember { mutableFloatStateOf(0f) }  // left edge of vowel
        var eRightX     by remember { mutableFloatStateOf(0f) }  // right edge of silent E

        val isMagicActive = uiState.segmentIndex == 1 || uiState.wordDone
        ...
        Column(
            ...
            // Arc canvas + char row stacked vertically
            Column(horizontalAlignment = Alignment.CenterHorizontally) {

                // Canvas reserves space for the arc above the characters
                Canvas(
                    modifier = Modifier
                        .fillMaxWidth()
                        .height(14.dp)
                    ...
                    if (vowelLeftX > 0f && eRightX > 0f) {
                        val fromX    = vowelLeftX - canvasRootX
                        val toX      = eRightX - canvasRootX
                        val midX     = (fromX + toX) / 2f
                        val arcColor = if (isMagicActive) accent else Color(0xFFD0D0D0)
                        val path = Path().apply {
                ...
                // Character row — vowel (idx 1) and silent E (idx 3) report their edges
                Row(
                    horizontalArrangement = Arrangement.Center,
                    verticalAlignment = Alignment.Bottom
                ) {
                    entry.magic.forEachIndexed { idx, ch ->
                        MagicCharCell(
                            char        = ch.toString(),
                            charIdx     = idx,
                            uiState     = uiState,
            ...
            // 3 segment dots: first-consonant · a-e (gold) · middle-consonant
            Row(
...
private fun MagicCharCell(
    ...
    val segIdx        = uiState.segmentIndex
    val isMagicPair   = segIdx == 1
    val isVowel       = charIdx == 1
    val isSilentE     = charIdx == 3
    val wordDone      = uiState.wordDone

    val isActive = (segIdx == 0 && charIdx == 0)
                || (isMagicPair && (isVowel || isSilentE))
                || (segIdx == 2 && charIdx == 2)

    val targetColor = when {
        wordDone -> accentColor
        isActive -> accentColor
        uiState.playedSegments.contains(
            when (charIdx) { 0 -> 0; 2 -> 2; else -> 1 }
        )        -> Color(0xFF37474F)
...
@Composable
private fun MagicEBottomControls(
    uiState: MagicEListenUiState,
    wordIndex: Int,
    totalWords: Int,
    accent: Color,
