...
class EndingBlendsPracticeViewModel @Inject constructor(
    ...
    private val questions = endingBlendsPracticeQuestions.shuffled().take(10)
    ...
    private fun advance() {
        ...
        uiState = if (next >= questions.size) {
            levelProgressRepo.recordPractice(level = PhonicsListenLevelKey.endingBlends, score = uiState.score, total = questions.size)
            phonicsSessions.recordPractice(PhonicsListenLevelKey.endingBlends, uiState.score, questions.size, ((System.currentTimeMillis() - sessionStartMs) / 1000).toInt(), wrongWords.toList(), correctWords.toList())
